...
import { getConfig, getPage } from "~/backend/loaders.server.js";
import {
  useLoaderData,
  useRouteError,
...
export const meta: V2_MetaFunction<typeof loader> = ({
  ...
  return getMetaTagsForArticle({
    ...
    title: page?.title
      ? `${page.title}${siteTitle ? ` - ${siteTitle}` : ""}`
      : siteTitle,
    description:
      page?.description ??
      project?.description ??
...
export default function Page() {
  ...
  const pageDesign: TemplateOptions =
    (data.page.frontmatter as any)?.site ?? {};
      ...
      <ProjectProvider>
        ...
        >
          <ThebeLoaderAndServer baseurl={baseurl}>
            <main
              ref={container}
              className="article-grid subgrid-gap col-screen"
            >
              <ArticlePage
                article={data.page}
